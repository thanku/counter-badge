...
function range(from, to) {
  return Array.from({ length: Math.abs(from - to) + 1 }, (_, i) => from + i);
}
...
function renderData({
  ...
  return html`
    ...
    >
      ${{
        __html: range(0, STEP_COUNT - 1).map(
          (i) =>
...
function restartStepperCmd(state, dispatch) {
  ...
  startStepperCmd({ ...state, intervalId: null }, dispatch);
}
...
function update(state, { name, payload }) {
  switch (name) {
    case "INIT": {
      const slug = isValidSlug(payload.slug) ? payload.slug : state.slug;
      const lang = isValidLang(payload.lang) ? payload.lang : state.lang;
      const duration = payload.duration > 0 ? payload.duration : state.duration;
...
class ThankUCounterBadge extends HTMLElement {
  ...
  set duration(duration) {
    const payload = Number.parseInt(duration);
    if (payload > 0) {
      this.dispatch({ name: "DURATION_UPDATED", payload });
    }
  }
